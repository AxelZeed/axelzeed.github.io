...
const CATALOG = [
  ...
  { author: 'SUPER_STICKER', type: 'supersticker', message: 'SUPER STICKER DEPLOYED! WOW!', avatar: AVATARS[3] },
...
function msgToHTML(msg: { author: string; type: string; message: string; avatar: string }) {
  const badge = msg.type === 'owner' ? '👑' : msg.type === 'moderator' ? '🔧' : msg.type === 'member' ? '⭐' : msg.type === 'superchat' ? '💸' : msg.type === 'supersticker' ? '🎁' : '';
  ...
  if (msg.type === 'superchat') {
    return `<yt-live-chat-paid-message-renderer author-type="member">
      <yt-img-shadow id="author-photo" class="yt-live-chat-text-message-renderer"><img id="img" src="${msg.avatar}" alt="" class="yt-live-chat-text-message-renderer"></yt-img-shadow>
      <div id="card">
        <div id="header">
          <yt-live-chat-author-chip>${badgeHTML}<span id="author-name">${msg.author}</span></yt-live-chat-author-chip>
  ...
  if (msg.type === 'supersticker') {
    return `<yt-live-chat-paid-sticker-renderer author-type="member">
      <yt-img-shadow id="author-photo" class="yt-live-chat-text-message-renderer"><img id="img" src="${msg.avatar}" alt="" class="yt-live-chat-text-message-renderer"></yt-img-shadow>
      <div id="card">
        <div id="content">
          <yt-live-chat-author-chip>${badgeHTML}<span id="author-name">${msg.author}</span></yt-live-chat-author-chip>
  ...
  const at = msg.type !== 'default' ? ` author-type="${msg.type}"` : '';
...
yt-img-shadow { display: inline-block; vertical-align: top; flex-shrink: 0; margin-right: 8px; }
yt-img-shadow img { width: 24px; height: 24px; border-radius: 50%; }
  ...
  return (
    <div className="w-full bg-[#020a0c] border border-white/10 rounded-lg overflow-hidden flex flex-col h-[650px] shadow-2xl relative">
      ...
      <div className="flex border-b border-white/5 bg-[#041215] relative z-20">
        ...
        <button onClick={() => refreshPreview(true)} className="px-4 border-l border-white/5 text-gray-500 hover:text-neon-cyan transition-colors" title="Refresh preview">
          <RefreshCcw size={16} />
        </button>
      </div>
      ...
      {tab === 'live' && (
        <div className="absolute top-16 right-4 z-30 flex flex-col items-end gap-2">
          <div className="flex gap-2">
            ...
            <button onClick={() => refreshPreview(true)} className="p-3 rounded-full border border-white/20 bg-black/50 text-white transition-all hover:border-neon-cyan" title="Reset simulation">
              <RefreshCcw size={18} />
            </button>
          </div>
          {showCtrl && (
